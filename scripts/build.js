...
function getPackageInputPath(package) {
    return path.resolve(packagesPath, `${package.name}/${SRC_FOLDER}`, package.inputFile)
}

function getPackageOutputPath(package) {
    return path.resolve(packagesPath, `${package.name}/${BUILD_FOLDER}`, package.outputFile)
}

function getPackageJson(package) {
    return JSON.parse(readFileSync(path.resolve(packagesPath, package.name, "package.json")))
}
